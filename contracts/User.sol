...
contract User {
  ...
  address private momAddress;  
  constructor(User) public {
    addresses.push(msg.sender);
    usernames.push('self');
    ipfsHashes.push('not-available');
  }
  function setMomAddress(address MomAddress) public{
    momAddress = MomAddress;    
  }
  
  function getMomAddress() public view returns(address MomAddress){
    return momAddress;
  }
  ...
  function createUser(bytes16 username, bytes ipfsHash) public returns(bool success){
    ...
    ////初始化Moment
    require(momAddress!=0, 'Moments Address is not inited');
    Moment Moments = Moment(momAddress);
    Moments.initMoment(username);
    return true;
  }
